...
TEST_F(inst, decode_global_store_b32) {
    // global_load_b32 v1, v0, s[4:5]
    inst_init(0xDC6A0000, 0x00060100);
    auto res = dynamic_pointer_cast<rinst_issue>(decode());

    EXPECT_EQ(res->type, FLAT);
    EXPECT_EQ(res->op, 26);
    EXPECT_EQ(res->src0_id, 0);
    EXPECT_EQ(res->src1_id, 6);
    EXPECT_EQ(res->vdata, 1);
}

TEST_F(inst, exe_global_store_b32) {
    rinst_issue issue{};
    issue.type = FLAT;
    issue.op = 26;
    //store the data to address 0x123
    issue.src1[0] = 0x123;
    issue.src1[1] = 0;
    issue.src0[0] = 2; //offsest
    //store data 3 to vram addr 0x125 in exe;
    issue.src2[0] = 3;

    auto res = dynamic_pointer_cast<rwriteback_t>(exe(&issue));

    auto vram_data = get_vram_data(0x123 + 2, 1);

    EXPECT_EQ(vram_data[0], 3);
}